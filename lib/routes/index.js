	...
	Routes = {
		...
		Products: [
			...
			// /product/detail/{id}
			{
				...
				config: {
					...
					description: 'Select an product detail',
					notes: 'Fetches an existing product',
					tags: ['api', 'product'],
					validate: {
						params: {
							id: Joi.number().min(1).max(9999).required()
		...
		Shop: [
			// /shop/list
			{
				...
				config: {
					auth: false,
					handler: Handlers.Shop.List,
					description: 'Select a shop list',
					notes: 'Fetches a list of shops',
					tags: ['api', 'shop']
				}
			},
			// /shop/detail/{id}
			{
				...
				config: {
					auth: false,
					handler: Handlers.Shop.Detail,
					description: 'Select an shop detail',
					notes: 'Fetches an existing shop',
					tags: ['api', 'shop'],
					validate: {
						params: {
							id: Joi.number().min(1).max(9999).required()
			...
			// /shop/insert
			{
				...
				config: {
					...
					handler: Handlers.Shop.Insert,
					description: 'Insert a new shop',
					notes: 'Insert a new shop',
					tags: ['api', 'shop'],
					validate: {
						payload: {
							name: Joi.string().required(),
			...
			// /shop/update
			{
				...
				config: {
					auth: {
						scope: [ 'public', 'god','admin', 'merch', 'merch_admin' ]
					},
					handler: Handlers.Shop.Update,
					description: 'Update an existing shop',
					notes: 'Update an existing shop',
					tags: ['api', 'shop'],
					validate: {
						payload: {
							id: Joi.number().required(),
			...
			// /shop/delete/{id}
			{
				...
				config: {
					...
					handler: Handlers.Shop.Delete,
					description: 'Deletes a shop',
					notes: 'Deletes an existing shop by the id parameter',
					tags: ['api', 'shop'],
					validate: {
						params: {
							id: Joi.number().min(1).max(9999).required()

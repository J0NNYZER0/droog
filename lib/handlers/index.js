	...
	Handlers = {
		Account: {
			...
			Detail: (request, reply) => {
				...
				return Accounts.findOne({ where: { id: request.params.id } })
					.then(data => reply({ status: 200, data }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Insert: (request, reply) => {

				return Accounts.create(request.payload)
			...
			Login: (request, reply) => {
				...
				if (!request.payload.username ||
					...
					.then(data => {
						...
						account = (data) ? data.dataValues : null
					...
					.catch(err => {
						...
						return reply({ status: 500, errorMessage: err, data: [] })})
				}
			},
		...
		Bag: {
			...
			Insert: (request, reply) => {
				...
				return Bags.create(request.payload)
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Update: (request, reply) => {
		...
		User: {
			...
			Update: (request, reply) => {
				...
				return Users.update(request.payload, { where: { id: request.payload.id }})
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Delete: (request, reply) => {
		...
		Views: {
			...
			Home: (request, reply) => {
				...
				reply.view('home', {
					...
					username: request.auth.credentials.user_name,

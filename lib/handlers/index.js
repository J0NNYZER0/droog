	...
	Handlers = {
		...
		BagItem: {
			...
			Upsert: (request, reply) => {
				...
				return BagItems.upsert(request.payload)
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Delete: (request, reply) => {
		...
		Customer: {
			...
			Upsert: (request, reply) => {
				...
				return Customers.upsert(Object.assign(request.payload, { updated_date: Utils.CreateTimestamp }))
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Delete: (request, reply) => {
		...
		CustomerOrder: {
			...
			Upsert: (request, reply) => {
				...
				return CustomersOrders.upsert(Object.assign(request.payload, { updated_date: Utils.CreateTimestamp }))
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Delete: (request, reply) => {
		...
		OrderStatus: {
			...
			Upsert: (request, reply) => {
				...
				return OrderStatus.upsert(request.payload)
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Delete: (request, reply) => {
		...
		Payment: {
			...
			Upsert: (request, reply) => {
				...
				return Payments.upsert(request.payload)
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Delete: (request, reply) => {
		...
		User: {
			...
			Detail: (request, reply) => {
				...
				return Users.findOne({ where: { id: request.params.id } })
					.then(data => reply({ status: 200, data }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Upsert: (request, reply) => {

				return Users.upsert(request.payload)
					.then(created => reply({ status: 200, created }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			},
			Delete: (request, reply) => {

				return Users.destroy({ where: { id: request.params.id } })
					.then(affected => reply({ status: 200, affected }))
					.catch(err => reply({ status: 500, errorMessage: err, data: [] }))
			}
		}

...
lab.experiment('Routes', () => {
	...
	lab.experiment('Home', () => {
		...
		lab.test('array length is 1', done => {
	...
	lab.experiment('Bags', () => {
		...
		lab.test('array length is 4', done => {
	...
	lab.experiment('Inventory', () => {

		lab.test('array length is 1', done => {

			Code.expect(Routes.Inventory.length).to.equal(1)
			done()
		})
	})

	lab.experiment('Orders', () => {
	...
	lab.experiment('Products', () => {
		...
		lab.test('array length is 1', done => {
			...
			Code.expect(Routes.Products.length).to.equal(1)
			done()
		})
	})
})
...
lab.experiment('Server', () => {
	...
	lab.experiment('is a local instance', () => {
		...
		lab.test('at the address http://0.0.0.0:8000', done => {
...
lab.experiment('bag/list - ', () => {
	...
	lab.experiment('Repsonse', () => {
		...
		lab.experiment('Data', () => {
			...
			lab.test('array contains an object containing the properties id, product_id, email, gender, size, quantity, release, created_date', done => {
				...
				Server.inject('/test/bag/list', (res) => {
					...
					Code.expect(dataObject.created_date, 'created_date').to.be.a.string()
					done()
				})
			})
...
lab.experiment('bag/{id} - ', () => {

	lab.test('is a GET request', done => {

		Server.inject('bag/1911', (res) => {
	...
	lab.experiment('When HTTP request contains a parameter', () => {
		...
		lab.experiment('Request', () => {
			...
			lab.test('contains a parameter which is a number', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
		...
		lab.experiment('Response data', () => {

			lab.test('contains an property called id of type number', done => {

				Server.inject('/test/bag/1911', (res) => {

					Code.expect(res.result.data.id, 'id').to.be.a.number()
					done()
				})
			})
			...
			lab.test('contains an property called property_id of type number', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
					...
					Code.expect(res.result.data.product_id, 'product_id').to.be.a.number()
					done()
				})
			})
			...
			lab.test('contains an property called email of type string', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
					...
					Code.expect(res.result.data.email, 'email').to.be.a.string()
					done()
				})
			})
			...
			lab.test('contains a property called gender of type string', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
					...
					Code.expect(res.result.data.gender, 'gender').to.be.a.string()
					done()
				})
			})
			...
			lab.test('contains a property called size of type string', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
					...
					Code.expect(res.result.data.size, 'size').to.be.a.string()
					done()
				})
			})
			...
			lab.test('contains a property called quantity of type number', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
					...
					Code.expect(res.result.data.quantity, 'quantity').to.be.a.number()
					done()
				})
			})
			...
			lab.test('contains a property called release of type number', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
					...
					Code.expect(res.result.data.release, 'release').to.be.a.number()
					done()
				})
			})
			...
			lab.test('contains a property called created_date of type string', done => {
				...
				Server.inject('/test/bag/1911', (res) => {
					...
					Code.expect(res.result.data.created_date, 'created_date').to.be.a.string()
					done()
				})
			})
		})
